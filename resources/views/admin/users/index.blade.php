    ...
    <div class="p-6 space-y-6" x-data="{ 
        ...
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 w-full text-left">
            <div class="flex flex-col gap-0.5">
                <h2 class="text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-55 font-nasalization">Manajemen Pengguna</h2>
        ...
        <div class="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm overflow-hidden text-left">
            <div class="overflow-x-auto">
                <table class="w-full text-xs">
                    <thead class="bg-slate-50/70 dark:bg-slate-900/50 border-b border-slate-150 dark:border-slate-800 text-slate-400 dark:text-slate-550 font-bold uppercase tracking-wider text-[10px]">
                        <tr>
                            ...
                            <tr class="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition-colors">
                                <td class="px-6 py-4 text-left">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <button @click="openDetail({{ json_encode($user) }})" class="font-bold text-slate-900 dark:text-slate-550 hover:text-indigo-600 dark:hover:text-indigo-400 text-left hover:underline">
                                                {{ $user->name }}
                                            </button>
                                            @if(auth()->id() === $user->id)
                                ...
                                <td class="px-6 py-4 text-center">
                                    ...
                                    @elseif(str_starts_with($user->role, 'admin_'))
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-indigo-50 dark:bg-indigo-955/20 text-indigo-700 dark:text-indigo-400 border border-indigo-150 dark:border-indigo-900/30 uppercase">Admin Unit</span>
                                    @elseif($user->role === 'kepala_sekolah')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-emerald-50 dark:bg-emerald-955/20 text-emerald-700 dark:text-emerald-400 border border-emerald-150 dark:border-emerald-900/30 uppercase">Kepala Sekolah</span>
                                    @elseif($user->role === 'waka')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded text-[9px] font-bold bg-amber-50 dark:bg-amber-955/20 text-amber-700 dark:text-amber-455 border border-amber-150 dark:border-amber-900/30 uppercase">Waka</span>
                                    @else
                                ...
                                <td class="px-6 py-4 text-right flex justify-end gap-2">
                                    ...
                                    @if(auth()->id() !== $user->id)
                                        <form action="{{ route('users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus user ini?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="w-8 h-8 rounded-lg bg-rose-50/40 hover:bg-rose-50 dark:bg-rose-955/10 dark:hover:bg-rose-955/20 border border-rose-150/40 dark:border-rose-900/30 text-rose-600 dark:text-rose-455 flex items-center justify-center transition-colors cursor-pointer" title="Hapus Akun">
        ...
        <div x-show="showAddModal" class="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-xs text-left" style="display: none;">
            <div @click.outside="showAddModal = false" class="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl max-w-md w-full overflow-hidden text-xs">
                <div class="p-5 border-b border-slate-150 dark:border-slate-850 flex justify-between items-center">
                    <h3 class="text-sm font-bold text-slate-900 dark:text-slate-50 font-nasalization">Tambah User Baru</h3>
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <input type="password" name="password" required placeholder="Minimal 8 karakter..." 
                                class="w-full h-9 px-3 bg-white dark:bg-slate-905 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800">
                        </div>
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="role" x-model="selectedRole" class="w-full h-9 px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800 cursor-pointer">
                                ...
                                <option value="admin_paud">Admin PAUD</option>
                        ...
                        <!-- Employee Link (Conditionally shown if role is employee) -->
                        <div class="space-y-1.5" x-show="selectedRole === 'employee'">
            ...
            <div @click.outside="showEditModal = false" class="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl max-w-md w-full overflow-hidden text-xs">
                ...
                <form :action="`{{ url('users') }}/${editUser.id}`" method="POST">
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="space-y-1.5">
                            <label class="block font-bold text-slate-700 dark:text-slate-300">Hak Akses (Role)</label>
                            <select name="role" x-model="editUser.role" class="w-full h-9 px-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-100 dark:focus:ring-slate-800 cursor-pointer">
                                <option value="employee">Pegawai (Employee)</option>
                                <option value="kepala_sekolah">Kepala Sekolah</option>
                                <option value="waka">Waka</option>
                                <option value="admin_paud">Admin PAUD</option>
                        ...
                        <!-- Employee Link (Conditionally shown if role is employee) -->
                        <div class="space-y-1.5" x-show="editUser.role === 'employee'">
                            <label class="block font-bold text-slate-700 dark:text-slate-300">Hubungkan ke Data Pegawai</label>
            ...
            <div @click.outside="showDetailModal = false" class="bg-white dark:bg-slate-955 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xl max-w-md w-full overflow-hidden text-xs">
                ...
                <div class="p-5 space-y-6">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-16 h-16 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 text-indigo-650 dark:text-indigo-400 font-bold flex items-center justify-center text-2xl uppercase shadow-sm">
                            <span x-text="selectedUser ? selectedUser.name.substring(0,2) : ''"></span>
                        </div>
                        <div class="space-y-1">
                            <h4 class="text-sm font-bold text-slate-900 dark:text-slate-55 font-nasalization" x-text="selectedUser ? selectedUser.name : ''"></h4>
                    ...
                    <div class="space-y-3">
                        ...
                        <div x-show="selectedUser && selectedUser.employee" class="bg-slate-50/50 dark:bg-slate-900/30 border border-slate-150 dark:border-slate-850 rounded-xl p-4 space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3 text-[11px] pt-2 border-t border-slate-200/40 dark:border-slate-800/40">
                                ...
                                <div>
                                    ...
                                    <span class="font-bold text-slate-750 dark:text-slate-300" x-text="selectedUser && selectedUser.employee ? (selectedUser.employee.status || '-') : '-'"></span>
                                </div>
                                <div>
                                    <span class="block text-slate-400 text-[9px] uppercase font-semibold">Terdaftar Pada</span>
                                    <span class="font-mono text-slate-655 dark:text-slate-400" x-text="selectedUser && selectedUser.employee ? new Date(selectedUser.employee.created_at).toLocaleDateString('id-ID', {day: 'numeric', month: 'short', year: 'numeric'}) : '-'"></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div x-show="selectedUser && !selectedUser.employee" class="border border-dashed border-slate-200 dark:border-slate-800 rounded-xl p-4 text-center text-slate-400">
                            <i data-lucide="link-2-off" class="w-5 h-5 mx-auto text-slate-350 dark:text-slate-750 mb-1"></i>
                ...
                <div class="p-5 border-t border-slate-150 dark:border-slate-850 flex justify-end">
                    <button @click="showDetailModal = false" class="h-9 px-4 bg-slate-900 dark:bg-slate-100 hover:bg-slate-850 dark:hover:bg-slate-205 text-white dark:text-slate-900 font-semibold rounded-lg shadow-sm transition-colors cursor-pointer">Tutup</button>
                </div>
            </div>
        </div>
