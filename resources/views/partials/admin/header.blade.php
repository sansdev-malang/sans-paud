<header id="header"
    ...
    <div class="flex items-center gap-3">
        ...
        <nav
            ...
            <span class="hover:text-slate-700 dark:hover:text-slate-350 cursor-pointer">{{ setting('unit_name', 'SANS Malang') }}</span>
            <span class="text-slate-300 dark:text-slate-700">/</span>
            <span class="hover:text-slate-700 dark:hover:text-slate-350 cursor-pointer font-bold">{{ ucwords(str_replace(['_', 'hrd', 'smp', 'sd', 'paud', 'employee'], [' ', 'HRD', 'SMP', 'SD', 'PAUD', 'Pegawai'], auth()->user()->role ?? 'Admin')) }}</span>
        </nav>
    </div>
    ...
    <div class="flex items-center gap-2">
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" id="notify-btn"
                ...
                @if(
                    (isset($pendingLeavesCount) && $pendingLeavesCount > 0) || 
                    (isset($myNotifications) && count($myNotifications) > 0)
                )
                        ...
                        @if(isset($myNotifications) && count($myNotifications) > 0)
                            @foreach($myNotifications as $item)
                                <a href="{{ route('my-leaves.index') }}" class="flex items-start gap-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-900/40 transition-colors">
                                    <div class="w-8 h-8 rounded-lg {{ $item->status === 'Approved' ? 'bg-emerald-50 dark:bg-emerald-955/20 text-emerald-700 dark:text-emerald-400 border border-emerald-100/50 dark:border-emerald-900/30' : 'bg-rose-50 dark:bg-rose-955/20 text-rose-700 dark:text-rose-455 border border-rose-100/50 dark:border-rose-900/30' }} flex items-center justify-center shrink-0">
                                    ...
                                    <div class="space-y-0.5 overflow-hidden">
                                        ...
                                        <p class="text-slate-500 dark:text-slate-405 text-[10px] truncate">Jenis: {{ $item->type }} tanggal {{ $item->start_date->format('d M Y') }}.</p>
                                    </div>
                                </a>
                            @endforeach
                        @else
                            <div class="p-6 text-center text-slate-400">
                                <i data-lucide="bell-off" class="w-6 h-6 mx-auto mb-1 text-slate-300 dark:text-slate-750"></i>
                                <p class="text-[10px]">Belum ada pembaruan status izin.</p>
                            </div>
                        @endif
                    @endif
                </div>
